...
def encode(tokenizer, subreddit_dict, input_dict, max_length, clause_max_length):
    ...
    # ========== 核心修改：替换缺失的sarcasm字段逻辑 ==========
    # 原代码访问input_dict["sarcasm"]报错，改用clauses拼接成完整文本

    sarcasm = " ".join(input_dict["clauses"])  # 分句拼接为完整文本
    sarcasm = sarcasm.strip()  # 去除首尾空格
    sarcasm_words = sarcasm.split()  # 拆分为单词列表
