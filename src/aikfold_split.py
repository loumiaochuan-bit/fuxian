...
samples = json.load(open("../data/reddit/ai_data.json", encoding="utf-8"))
...
for not_test_index, test_index in kf.split(X=new_samples, y=subreddits, groups=subreddits):
    ...
    if not os.path.exists("../data/reddit/aisplit/"):
        os.makedirs("../data/reddit/aisplit/")
    split_dir = os.path.join("../data/reddit/aisplit/", str(i))
